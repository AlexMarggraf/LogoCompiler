...
To know what we had to implement, it is good to have an overview of what constructs the turtle graphics language Logo supports. 
...
\begin{tabular}{|p{4.7cm}|p{10.4cm}|}
    ...
    \hline
    \texttt{to $n$ $p$ \newline \phantom{mm}$s$ \newline end}& declare a procedure with name $n$ which executes the statements $s$. $p$ is the list of parameters accepted by that procedure which can be used by the statements $s$. Procedures cannot return anything. The entry point of the source code in our implementation is the procedure with the name \texttt{main}.  \\
    \hline
...
Our implementation consists of a compiler that compiles from Logo to JavaScript. 
The compiled code uses an object that we call an action set. 
This action set defines what happens for each basic command in the Logo code. 
This means that for each basic command, a method with the same name exists on the action set, which defines its behavior. 
...
\\\\
Note that all the constructs in Logo have matching constructs in JavaScript\footnote{\texttt{repeat} gets transformed to a \texttt{for} loop, \texttt{stop} to a \texttt{return} or \texttt{break} based on the context it appears in and procedures to functions}. 
...
The basic commands (see Section \ref{sec:basic-commands}) are mapped to action set calls in the compiler. How those are generated depends on the strategy the compiler is configured with. 
...
\begin{itemize}
  \item The parameters of a method become local variables that get declared and initialized at the beginning of the resulting call site. 
  \item Member expressions on \texttt{this} (which represents the action set) get modified to have the \texttt{\_\_act\_} prefix instead of \texttt{this.}.
  \item All of this code gets wrapped in a scope to not redefine the same variable if a method gets called twice\footnote{This could be circumvented by renaming the local variables, but due to time constraints we weren't able to do that}. 
\end{itemize}
To see this in action, we take a look at what happens when transforming the call \texttt{rt 100} to a \emph{hard coded} call. 
...
\end{lstlisting}
If inside a method there is a call to a member of \texttt{this}, the same rule is applied. We opted not to recursively hard code these calls as well, since recursion would pose a problem. 
...
In the following example we transform the call \texttt{fd :l}.
...
Our approach is to compile the action set so that fields become global variables and methods become functions, which all have the required prefix. 
We then preface the rest of the code with this version of the action set code, such that all the necessary variables and functions are defined. 
\\\\
...
\texttt{eval} inherits the scope of the caller, which means that we can ``pass'' the action set by just defining a variable with the same name as is used in the compiled code and then calling \texttt{eval} (see first part of Listing \ref{lst:eval_vs_function}).
However, \texttt{eval} does not work well with bundlers\footnote{we use esbuild as a bundler for our project} and is quite error-prone because of the implicit inclusion of all these objects. 
\\\\
So we opted to use the \texttt{Function} class.
Using the \texttt{Function} class, one can construct a callable object that executes arbitrary code, but it behaves like a normal function.
This means that one can define parameters that have to be supplied to the callable explicitly (see the second part of Listing \ref{lst:eval_vs_function}). 
In our case, the action set and the canvas context have to be supplied to this callable object. 
\\\\
...
After each wait call, it is checked whether the value of the stopper object has changed, and if so, the program is terminated. 
Such a stopper object can then be used to stop running code in an application. 