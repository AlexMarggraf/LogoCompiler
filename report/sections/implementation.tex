...
\\\\
Note that all of the constructs in Logo have matching constructs in javascript\footnote{\texttt{repeat} gets transformed to a \texttt{for} loop, \texttt{stop} to a \texttt{return} or \texttt{break} based on the context it appears in and procedures to functions}. 
...
\end{lstlisting}
If inside a method, a method call to \texttt{this} happens, the same rule is applied. We opted not to recursively hard code these calls as well, since recursion would pose a problem. 
...
This means that one can define parameters which have to be supplied to the callable explicitly (see second part of Listing \ref{lst:eval_vs_function}). 
