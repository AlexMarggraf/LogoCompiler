...
Our compiler implements various approaches as to how the compiled code looks. We compare the performance of those results. 
...
What follows is an introduction to polymorphic inline caches. 
...
\subsection{Overview}
From this point on, we only consider object-oriented languages that are compiled to machine code (either JIT or AOT).
...
\\\\
The first optimization one can make is to store the results of this lookup procedure in a simple lookup cache (lookup table) that maps the type of the receiver together with the name of the message to the address of the corresponding machine code. 
...
This optimization improves performance considerably. (\cite{pics})
\\\\
...
When an inline cache miss happens, the correct procedure has to be found - either through the lookup table or the lookup procedure - and its address inserted at the call site. 
...
So at each call site, there is conceptually a switch statement on the type of the receiver, which has call instructions with the procedure corresponding to that type. 
...
\\\\
What happens in modern JavaScript engines is that depending on the behavior of a call site, different methods of caching are employed. 
